...
async function assert_subscriptions_data() {
    ...
    for (var i = 0; i < data[4].data.getAllSubscriptions.length; i++) {
        // API Data & Variables
        var sub_start_date = await ActionsPage.format_date(data[4].data.getAllSubscriptions[i].item1.subscriptionStartDate)
        ...
        var sub_end_date = await ActionsPage.format_date(data[4].data.getAllSubscriptions[i].item1.subscriptionEndDate)
        var sub_end_day = sub_end_date.getDate()
        var sub_end_month = sub_end_date.getMonth()
        var sub_end_year = sub_end_date.getFullYear()
        var api_assigned_payment = data[4].data.getAllSubscriptions[i].item2.friendlyName
        var months = {
            0: "January", 1: "February", 2: "March", 3: "April", 4: "May", 5: "June", 6: "July", 7: "August", 8: "September", 9: "October", 10: "November", 11: "December",
        }
        var sub_end_month = months[sub_end_month]
        var api_valid_until = sub_end_month + " " + sub_end_day + ", " + sub_end_year
        if (data[4].data.getAllSubscriptions[i].item1.friendlyName == null) {
            ...
            var api_contract_name = data[4].data.getAllSubscriptions[i].item1.friendlyName
        }
        if (data[4].data.getAllSubscriptions[i].item1.autopay == true) {
            var api_autopay = "On"
        }
        else {
            var api_autopay = "Off"
...
async function edit_contract(payment_method, autopay) {
    ...
    if (payment_method == "first") {
        ...
        var friendly_name = data[5].data.getPaymentMethods[0].friendlyName
    }
    if (payment_method == "last") {
        var payment_methods_lenght = data[5].data.getPaymentMethods.length
        var payment_method_id = payment_methods_lenght - 1
        var friendly_name = data[5].data.getPaymentMethods[payment_method_id].friendlyName
    }
    ...
    if (data[4].data.getAllSubscriptions[0].item1.autopay == false && autopay == "On" ||
        data[4].data.getAllSubscriptions[0].item1.autopay == true && autopay == "Off") {

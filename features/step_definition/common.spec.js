...
Given('I am in Scorpion login page', { timeout: 500000 }, async function () {
    await ActionsPage.navigate("https://login-integration.scorpion.co/")
});

Given('I am in Scorpion {string} page', { timeout: 500000 }, async function (url) {
    await ActionsPage.navigate("https://ui-integration.scorpion.co/" + url)
});

When('I wait for {string} seconds', { timeout: 500000 }, async function (seconds) {
    await ActionsPage.wait(seconds)
});

When('I click on settings button', { timeout: 500000 }, async function () {
    await ActionsPage.click_element(MainPageLocator.settings_button())
});

Then('I select the {string} option', { timeout: 500000 }, async function (option) {
    await ActionsPage.click_element_from_list(MainPageLocator.settings_options(), option)
});

When('I sign out Scorpion', { timeout: 500000 }, async function () {
    await ActionsPage.click_element_from_list(MainPageLocator.settings_options(), "Sign Out")
});

When('I hover on More option', { timeout: 500000 }, async function () {
